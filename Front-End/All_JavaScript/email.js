async function addNewsletter(event) {
  ...
  let url = "http://localhost:42480/api/Newsletter"; // API endpoint
...
function searchBook() {
  ...
  timeout = setTimeout(() => {
    ...
    if (bookTitle) {
      const apiUrl = `http://localhost:42480/api/Books/bynameBooks/Book%20Title%201/${encodeURIComponent(
        bookTitle
      )}`;
...
function displayResults(data) {
  ...
  if (data.length > 0) {
    data.forEach((book) => {
      ...
      resultItem.onclick = () => {
        window.location.href = `http://localhost:42480/api/Books/details/${encodeURIComponent(
          book.title
        )}`;
      };
